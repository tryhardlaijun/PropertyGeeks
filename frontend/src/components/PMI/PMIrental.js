...
function PMIrent() {
	...
	useEffect(() => {
		async function getRentalPrice() {
			const response = await axios(
				`http://127.0.0.1:5001/pmi/filter/getPMIRentalDetails?pmi_id=${params.id}`
			);
	...
	const Record = (props) => (
		<TableRow>
			...
			<TableCell align="center">
				<p className="bodyText">${props.record.rent_price}</p>
			</TableCell>
		</TableRow>
	);
					...
					<table class="table table-striped text-center">
						...
						<tbody>
							...
							<tr>
								<td>
									<p className="bodyText">
									{housePrice[0]?housePrice[0]["area(Sqm)"]:""} sqm
									</p>
								</td>
								<td>
									<p className="bodyText">
